...
class MakeCreateRequest extends GeneratorCommand
{
    protected $name = 'make:senses_create_request';

    protected $description = 'Create a new create request';

    protected $type = 'Create Request';

    protected function getStub(): string
    {
        return $this->resolveStubPath('/stubs/generator/create-request.stub');
    }
    ...
    protected function buildUsingJson(&$stub)
    {
        ...
        $permissions = "\t\t" . 'if (getCurrentUser()?->can(\'create-' . $this->option('model') . '\')) {' . PHP_EOL . "\t\t\t" . 'return true;' . PHP_EOL . "\t\t" . '}';
            ...
            if (in_array($field['name'], $jsonData['fillables']) || $field['type'] === 'relationship') {
                if ($field['type'] !== 'morphs') {
                    ...
                    if (array_key_exists('unique', $field) && $field['unique']) {
                        array_push($additions, 'unique:' . Str::plural(str_replace('-', '_', $this->option('model'))) . ',' . $field['name']);
                    }
