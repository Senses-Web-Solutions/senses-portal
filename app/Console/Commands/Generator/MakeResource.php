...
{
    protected $name = 'make:senses_resource';
    ...
    {
        return $this->resolveStubPath('/stubs/generator/resource.stub');
    }
    ...
    protected function buildUsingJson(&$stub)
    {
        ...
        foreach($jsonData['fields'] as $field){
            if(!str_ends_with($field['name'], '_id')){
                $string = '\''. $field['name'] .'\' => $this->' . $field['name'] . ', ' . PHP_EOL;
            } else {
                $model = substr($field['name'], 0, -3);
                $string = '\''. $field['name'] .'\' => $this->' . $field['name'] . ', ' . PHP_EOL . "\t\t\t" . '\'' . Str::plural(Str::camel($model)) . '\' => new CollectionResource($this->whenLoaded(\'' . Str::plural(Str::camel($model)) . '\'), ' . ucfirst(Str::camel($model)) . 'Resource::class),' . PHP_EOL;
            }
            $fields .= "\t\t\t" . $string;
        }
    ...
    protected function getPath($name): string
    {
        ...
        return "{$this->laravel['path']}/../app/Http/Resources/$fileName";
    }
}
