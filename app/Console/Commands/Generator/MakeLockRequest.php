...
class MakeLockRequest extends GeneratorCommand
{
    protected $name = 'make:senses_lock_request';

    protected $description = 'Create a new lock request';

    protected $type = 'Lock Request';

    protected function getStub(): string
    {
        return $this->resolveStubPath('/stubs/generator/lock-request.stub');
    }
    ...
    {
        $permissions = "\t\t" . 'if (getCurrentUser()?->can(\'lock-' . $this->option('model') .'\')) {' . PHP_EOL . "\t\t\t" . 'return true;' . PHP_EOL . "\t\t" . '}';
