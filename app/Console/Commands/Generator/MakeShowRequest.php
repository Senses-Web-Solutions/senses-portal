...
class MakeShowRequest extends GeneratorCommand
{
    protected $name = 'make:senses_show_request';

    protected $description = 'Create a new show request';

    protected $type = 'Show Request';

    protected function getStub(): string
    {
        return $this->resolveStubPath('/stubs/generator/show-request.stub');
    }
    ...
    {
        $permissions = "\t\t" . 'if (getCurrentUser()?->can(\'show-'. $this->option('model') .'\')) {' . PHP_EOL . "\t\t\t" . 'return true;' . PHP_EOL . "\t\t" . '}';
