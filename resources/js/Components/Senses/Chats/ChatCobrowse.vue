<template>
    ...
    >
        <div
            class="p-3 border-b border-zinc-200 flex items-center justify-between bg-white"
        >
            <div class="text-black font-semibold text-xl">
                Cobrowsing with {{ chat?.chat_user?.full_name }}
            </div>

            <ButtonGroup>
                <SecondaryButton @click="togglePulseMode"
                    >{{ inPulseMode ? "Exit" : "Enter" }} Pulse
        ...
        >
            <video
                class="w-auto border border-zinc-200"
                id="video"
                autoplay
                @click="sendPulse"
            ></video>
...
export default {
    ...
    methods: {
        ...
        setupEchoListeners() {
            const chatChannel = echo.private(
                `companies.${user().company_id}.chat`
            );
