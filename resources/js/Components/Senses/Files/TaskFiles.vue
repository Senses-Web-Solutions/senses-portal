<template>
    ...
    <FileForm :fileables="formFileables"/>
</template>
<script>
import {
    ArrowUpIcon,
...
//todo make this generic, task version should only pass the data in? Its also got templates that rely on slots for info but uses url in the templates?  (component is and view stuff within each type!)
...
export default {
    ...
    setup(props) {
        ...
        axios.get('/api/v2/tasks/' + props.id + '/files?format=folders').then(response => files.value = response.data);
        ...
        const formFileables = [{id: props.id, type: "task"}];
